...
export class HomeService {
  ...
  async addMember(id, homeId, data: AddMemberDto) {
    ...
    const requestPending = await this.requestMemberService.getRequest({
      to: member.id,
      home: homeId,
      status: 'Pending',
    });
    ...
    await this.requestMemberService.createRequest({
      from: id,
      to: member.id,
      home: homeId,
    });
  ...
  async getHome(id, homeId) {
    const home: any = await this.homeModel
      .findById(homeId)
    ...
    const requestsPending = await this.requestMemberService.getRequestPending(
      homeId,
    );

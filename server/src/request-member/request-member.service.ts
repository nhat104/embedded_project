...
export class RequestMemberService {
  ...
  async getRequests(id, data: GetRequestsDto) {
    ...
    const requests = await this.requestMemberModel
      .find(findOption)
      .select('from home status createdAt')
      .skip(pageSize * (pageNumber - 1))
      .limit(pageSize)
      .populate('from', 'name phone')
      .populate('home', 'name address')
      .sort({ createdAt: -1 });
  ...
  async replyRequest(userId, id, data: ReplyRequestDto) {
    ...
    const notificationData = {
      content: `${user.name} ${
        status == StatusRequest.ACCEPTED ? 'accepted' : 'rejected'
      } your invitation to become a member of ${home.name} home`,
      user: request.from,
  ...
  async getRequestPending(homeId) {
    return await this.requestMemberModel
      .find({
        home: homeId,
